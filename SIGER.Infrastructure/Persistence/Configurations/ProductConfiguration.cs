...
public class ProductConfiguration : IEntityTypeConfiguration<Product>
{
    public void Configure(EntityTypeBuilder<Product> builder)
    {
        ...
        builder.Property(product => product.Description).HasColumnName("descripcion").HasMaxLength(500);
        builder.Property(product => product.Price).HasColumnName("precio").HasPrecision(12, 2).IsRequired();
        builder.Property(product => product.IsAvailable).HasColumnName("disponible").IsRequired();
        builder.Property(product => product.ImageUrl).HasColumnName("imagen_url").HasColumnType("text");
